...
function Dialog({kind,accounts,close,done,fail}:{kind:'send'|'faucet'|'mine';accounts:Account[];close:()=>void;done:(s:string)=>void;fail:(s:string)=>void}){
 ...
 {kind!=='mine'&&<><label>Destination account<select value={to} onChange={e=>setTo(+e.target.value)}>{accounts.map(a=><option value={a.id} key={a.id}>{a.name}</option>)}</select></label><label>Destination pool<select value={dest} onChange={e=>setDest(e.target.value as Pool)}><option value="orchard">Orchard shielded</option><option value="transparent">Transparent</option></select></label><label>Amount (ZEC)<input type="number" min="0.00000001" step="0.00000001" value={amount} onChange={e=>setAmount(e.target.value)}/></label></>}
 {kind==='mine'&&<label>Number of blocks<input type="number" min="1" max="10000" value={blocks} onChange={e=>setBlocks(+e.target.value)}/></label>}
 <div className="auto"><Pickaxe/><div><b>Auto-confirm</b><span>{kind==='mine'?'Blocks use Regtest instant mining.':'One block will be mined after this action.'}</span></div></div><button className="primary submit" disabled={busy}>{busy?'Working…':kind==='mine'?'Mine blocks':kind==='faucet'?'Add funds':'Review & send'}</button></form></div>
}
